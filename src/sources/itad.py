...
class ITADSource:
    ...
    async def fetch_free_games(self) -> List[Dict[str, Any]]:
        ...
        browser = None # اطمینان از مقداردهی اولیه browser
        try:
        ...
        except Exception as e: # این بلوک Exception کلی برای کل عملیات Playwright است
            logger.critical(f"🔥 یک خطای بحرانی پیش‌بینی نشده در ماژول ITAD (اسکرپینگ Playwright) رخ داد: {e}", exc_info=True)
            return []
        finally:
            if browser: # اطمینان حاصل کنید که browser شیء معتبری است قبل از بستن
                await browser.close()
