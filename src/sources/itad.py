...
class ITADSource:
    """
    Fetches deals from IsThereAnyDeal.com using its RSS feed.
    Uses Playwright to fetch the RSS content to ensure reliability against anti-bot measures.
    """
    ...
    async def _fetch_rss_with_playwright(self) -> Optional[str]:
        """Fetches the raw RSS content using Playwright."""
        browser = None
        try:
            async with async_playwright() as p:
                ...
                if response and response.ok:
                    ...
                    logger.error(f"❌ [{self.__class__.__name__}] Failed to fetch RSS feed, status: {status}")
                    return None
        except TimeoutError:
